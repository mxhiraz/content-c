// Playbook system prompt loaded from prompts/playbook-system.md via skills loader.
...
export async function refreshPlaybook(): Promise<Playbook> {
  const niche = process.env.NICHE ?? "AI (broad: applications in industry, hardware, policy, agents, robotics, consumer apps, controversy)";
  const today = new Date().toISOString().slice(0, 10);
  const sys = skills.playbook.system({ niche, today });
